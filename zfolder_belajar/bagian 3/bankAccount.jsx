...
function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'requestLoan':
      ...
      return { ...state, loan: action.amount, balance: state.balance + action.amount };
    case 'payLoan':
      if (state.loan === 0) {
        return state;
      }
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', textAlign: 'center' }}>
      ...
      <p>
        <button onClick={() => dispatch({ type: 'deposit', amount: 150 })} disabled={!state.isActive}>
          Deposit 150
        </button>
      </p>
      <p>
        <button onClick={() => dispatch({ type: 'withdraw', amount: 50 })} disabled={!state.isActive}>
          Withdraw 50
        </button>
      </p>
      <p>
        <button onClick={() => dispatch({ type: 'requestLoan', amount: 5000 })} disabled={!state.isActive}>
